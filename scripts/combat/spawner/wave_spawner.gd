...
func start_next_wave():
	if current_wave >= waves.size():
		print("Alle Wellen gespawnt!")
		return
	
	print("Starte Welle ", current_wave + 1)
	spawning = true
...
func spawn_wave(wave_data: WaveData):
	...
	spawn_pool.shuffle()
	spawn_enemies_with_delay(spawn_pool)
	pass
...
func spawn_enemies_with_delay(spawn_tasks: Array):
	...
	await get_tree().create_timer(waves[current_wave].spawn_delay_between_enemies).timeout
	spawn_enemies_with_delay(spawn_tasks)
	pass

func finish_wave():
	print("Welle ", current_wave + 1, " abgeschlossen.")
	spawning = false
	current_wave += 1
